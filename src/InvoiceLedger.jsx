...
//                   {/* NEW: Smart Status Column with Email Tracking */}
//                   <td style={styles.td}>
...
//                       {inv.status !== 'PAID' && (
//                         <button 
//                           onClick={() => setConfirmModal({ isOpen: true, action: 'EMAIL', targetId: inv.id, isBulk: false })} 
...
//                   <>
//                     <button onClick={() => setConfirmModal({ isOpen: true, action: 'EMAIL', targetId: drawerInvoice.id, isBulk: false })} style={{...styles.submitBtn, backgroundColor: drawerInvoice.emailed_at ? '#4B5563' : '#3B82F6'}}>
...
//       {/* ENTERPRISE UPGRADE: UNIFIED SMART MODAL OVERLAY */}
//       {confirmModal.isOpen && (
...
//   // MODAL STYLES (MATCHED TO ADMIN DASHBOARD / INVOICING HUB)
...
export default function InvoiceLedger() {
  ...
  const handleActionClick = (action, targetId, isBulk = false) => {
    // If the admin muted warnings this session (and it's not a destructive VOID), execute instantly!
  ...
  const confirmModalAction = async () => {
    // Save mute preference if checked (Only for Safe Actions like EMAIL and PAY)
    ...
    // 🔥 2. OPTIMISTIC UI UPDATE: Pretend it already succeeded!
    setInvoices(prev => {
      if (action === 'VOID') {
  ...
  const executeBackgroundAction = async (action, targets) => {
    try {
      ...
      // Silently fetch fresh data just to ensure total sync with the DB
      fetchInvoices();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {/* ENTERPRISE UPGRADE: UNIFIED SMART MODAL OVERLAY (With Session Mute) */}
      {confirmModal.isOpen && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalBox}>
            <h3 style={{ marginTop: 0, fontSize: '20px', color: confirmModal.action === 'VOID' ? '#DC2626' : '#111827' }}>
            ...
            {/* SESSION MUTE CHECKBOX (Hidden for dangerous VOID actions) */}
